...
@section('content-header')
    <h1>
        {{ trans('category::categories.title.create category') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.category.category.index') }}">{{ trans('category::categories.title.categories') }}</a></li>
        <li class="active">{{ trans('category::categories.title.create category') }}</li>
    </ol>
@stop
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('category::admin.categories.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
